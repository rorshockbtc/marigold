...
export function CryptographicNoteModal({ record, isOpen, onClose }: CryptographicNoteModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[99999] bg-slate-50 border border-slate-200/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto animate-fadeIn">
      <div className="bg-white dark:bg-slate-50 border border-slate-200 border-2 border-slate-200 dark:border-slate-200 rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden text-slate-900 dark:text-slate-100">
        {/* Header */}
        <div className="p-5 bg-amber-50 dark:bg-slate-100 border-b border-amber-200 dark:border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h3 className="font-black text-slate-900 dark:text-slate-900 text-lg">Community Field Note & Observation</h3>
              <p className="text-xs font-semibold text-slate-600 dark:text-slate-600">Shining light on civic data while protecting citizen privacy and community standards.</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 space-y-8 max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-100 dark:bg-slate-800/80 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div>
                <span className="text-[11px] font-extrabold text-slate-500 dark:text-slate-600 uppercase tracking-wider block">Target Location</span>
                <span className="font-extrabold text-base text-slate-900 dark:text-slate-900">{displayAddress}</span>
              </div>
              <span className="bg-white dark:bg-slate-50 border border-slate-200 text-slate-800 dark:text-slate-200 text-xs font-black px-3 py-1 rounded-lg border border-slate-300 dark:border-slate-700 shadow-sm">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-white dark:bg-slate-100 border-2 border-slate-300 dark:border-slate-700 rounded-xl p-3.5 text-sm font-medium text-slate-900 dark:text-slate-900 focus:border-amber-500 focus:outline-none placeholder:text-slate-600 shadow-inner"
              />
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
              <div className="space-y-0.5">
                <label className="block text-xs font-black uppercase text-slate-700 dark:text-slate-700">Volunteer Verification PIN</label>
                <p className="text-[11px] text-slate-500 dark:text-slate-600">A simple 4-digit number to tag your observation with your local team.</p>
              </div>
              <div className="flex items-center gap-3 self-end sm:self-center">
                <input
                  ...
                  placeholder="2026"
                  className="w-24 text-center bg-white dark:bg-slate-100 border-2 border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-base font-black font-mono tracking-widest text-slate-900 dark:text-slate-900 focus:border-amber-500 focus:outline-none shadow-sm"
                />
                <button
                  type="button"
            ...
            {encryptedPayload && (
              <div className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-950/30 border-2 border-emerald-500/40 rounded-xl space-y-3 animate-fadeIn">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-black text-emerald-800 dark:text-emerald-800 flex items-center gap-1.5">
                ...
                <pre className="text-xs font-mono font-bold text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-50 border border-slate-200 p-3.5 rounded-lg overflow-x-auto border border-emerald-500/30 shadow-inner">
                  {encryptedPayload}
                </pre>
          ...
          <div className="space-y-4 text-sm leading-relaxed text-slate-700 dark:text-slate-700 font-medium">
            <h4 className="font-extrabold text-slate-900 dark:text-slate-900 text-base flex items-center gap-2">
            ...
            <ul className="list-disc list-inside space-y-2.5 ml-1 text-slate-700 dark:text-slate-700">
              <li>
                <strong className="text-slate-900 dark:text-slate-900">No Names Broadcast:</strong> When you leave a field observation on a dormitory or apartment building, citizen names are never included.
              </li>
              <li>
                <strong className="text-slate-900 dark:text-slate-900">Anonymous Location ID:</strong> The street address is converted into an anonymized digital badge so volunteers can confirm locations without transmitting sensitive information.
              </li>
              <li>
                <strong className="text-slate-900 dark:text-slate-900">Simple Team PIN:</strong> Instead of complicated hexadecimal passwords, your local volunteer team uses a simple 4-digit verification code to group observations together.
              </li>
            </ul>
          </div>
          ...
          <div className="space-y-4 text-xs leading-relaxed text-slate-700 dark:text-slate-700">
            <h4 className="font-extrabold text-slate-900 dark:text-slate-900 text-base flex items-center gap-2">
            ...
            <div className="space-y-3 bg-slate-100 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-medium">
              <div>
                <span className="text-slate-500 dark:text-slate-600 block text-xs font-bold uppercase">Anonymization Method</span>
                <span className="text-slate-900 dark:text-slate-900 font-extrabold">One-Way SHA-256 Location Hashes</span>
              </div>
              <div>
                <span className="text-slate-500 dark:text-slate-600 block text-xs font-bold uppercase">Data Storage Location</span>
                <span className="text-slate-900 dark:text-slate-900 font-extrabold">Local Browser Memory & IndexedDB Sandbox</span>
              </div>
              <div>
                <span className="text-slate-500 dark:text-slate-600 block text-xs font-bold uppercase">Network Transmission</span>
                <span className="text-emerald-700 dark:text-emerald-700 font-extrabold">0.00% Automatic Exfiltration (User-Controlled Copy Only)</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="p-4 bg-slate-100 dark:bg-slate-100 border-t border-slate-200 dark:border-slate-200 flex justify-end">
          <button
            onClick={onClose}
            className="bg-slate-800 hover:bg-slate-700 text-slate-900 font-extrabold px-6 py-2.5 rounded-xl text-sm transition-colors shadow"
          >
            Done
          </button>
        </div>
