  ...
  return (
    <div className="fixed inset-0 z-[99999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden text-slate-900 dark:text-slate-100">
        ...
        {/* Navigation Tabs */}
        <div className="flex border-b border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/60 text-xs font-bold">
          <button
            onClick={() => setActiveTab('create')}
            className={`flex-1 py-3 border-b-2 transition-colors ${activeTab === 'create' ? 'border-amber-500 text-amber-800 dark:text-amber-300 bg-white dark:bg-slate-800 font-extrabold' : 'border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-900'}`}
          >
            ✍️ Write Observation
          </button>
          <button
            onClick={() => setActiveTab('plain-guide')}
            className={`flex-1 py-3 border-b-2 transition-colors ${activeTab === 'plain-guide' ? 'border-emerald-500 text-emerald-800 dark:text-emerald-300 bg-white dark:bg-slate-800 font-extrabold' : 'border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-900'}`}
          >
            📖 Plain-English Guide
          </button>
          <button
            onClick={() => setActiveTab('tech-spec')}
            className={`flex-1 py-3 border-b-2 transition-colors ${activeTab === 'tech-spec' ? 'border-blue-500 text-blue-800 dark:text-blue-300 bg-white dark:bg-slate-800 font-extrabold' : 'border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-900'}`}
          >
            🛡️ Privacy Details
          </button>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          {activeTab === 'plain-guide' && (
            <div className="space-y-4 text-sm leading-relaxed text-slate-700 dark:text-slate-300 font-medium">
              ...
              <h4 className="font-extrabold text-slate-900 dark:text-white text-base">How Community Collaboration Respects Privacy:</h4>
          ...
          {activeTab === 'tech-spec' && (
            <div className="space-y-4 text-xs leading-relaxed text-slate-700 dark:text-slate-300">
              <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-300 dark:border-blue-700 p-4 rounded-xl text-blue-950 dark:text-blue-200 font-bold text-sm">
                🛡️ <strong className="font-black">Zero Sensitive Information Architecture</strong> for community volunteers and state partners.
              </div>
