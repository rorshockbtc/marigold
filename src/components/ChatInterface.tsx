    ...
    try {
      ...
      while (response.ok && loopResponseData.action === 'run_tool' && loopCount < 5) {
        ...
        if (t === 'query_dataset' || t.startsWith('run_')) {
          ...
          const toolMessage: ChatMessage = { role: "user", content: `[SYSTEM: Title successfully updated.]` };
          loopMessages = [...loopMessages, toolMessage];
      ...
      const assistantMessage: ChatMessage = { 
        role: "assistant", 
        content: response.ok ? finalReply : `Error: ${loopResponseData.error || data.error}`,
