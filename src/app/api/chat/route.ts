...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemInstruction = `
      ...
      5. ZERO HALLUCINATION EXCEPTION: Do not invent fake rows or fake names for the local dataset. HOWEVER, if the user asks a general question outside the dataset (e.g. macroeconomic trends, public health, pigeon populations, historical data), YOU MUST DO TWO THINGS: First, use the 'triage_and_fetch_dataset' tool to provide a URL where they can find this public dataset. Second, USE YOUR BROAD TRAINING KNOWLEDGE to proactively generate a chart and a narrative section answering their question using 'append_section'. Do not refuse by saying you don't have internet access; rely on your LLM weights to generate the story!
      ...
      - IMPORTANT: If a user asks a broad analytical or historical question (e.g., 'What are national demographic trends from 1960 to today?'), evaluate if the currently linked dataset can answer it.
        ...
        - If NO or UNCERTAIN (the user is asking for national historical data, general knowledge, or data not in the local voter roll), DO NOT run 'query_dataset' against the local file! Instead, use your broad training knowledge to construct full, labeled multi-series charts (e.g. X: years/categories, Y: percentages/values) and use 'append_section' to update the Data Story canvas.
      
      MULTI-STEP REASONING & LOOPING:
      - You have the ability to run multiple tools in a loop! If you need to fetch data, call \`query_dataset\` first. Wait for the system to return the data. THEN, in the next turn, call \`append_section\` to visualize the data or write the narrative.
